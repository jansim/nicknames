...
test_that("labs_map works with .data", {
...
test_that("labs_map strips function calls when using .data", {
  p <- ggplot2::ggplot(
    mtcars,
    ggplot2::aes(
      .data[["wt"]],
...
test_that("labs_map works with composite columns when using .data", {
  p <- ggplot2::ggplot(mtcars, ggplot2::aes(.data[["wt"]] + .data[["mpg"]], mpg)) +
    ggplot2::geom_point()
